...
class DragNDropApp:
    def __init__(self, root):
        self.root = root
        self.root.title('Сервис перевозок')
        ...
        def on_button_release_card(self, event, data):
            if data['in_truck']:
                ...
                # Update the text in the Список рейсов window
                for item_id in self.tree_trips.get_children():
                    values = self.tree_trips.item(item_id, 'values')
                    card_number = values[0]
                    if card_number == str(data['number']):
                        self.tree_trips.item(item_id, values=(card_number, f"Добавлена в грузовик {data['in_truck']}"))
                        break
